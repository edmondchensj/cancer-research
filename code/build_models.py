...
def get_top3_scores_and_index(coherence_values,num_topics_range,cutoff):
    '''
...
def get_topn_models(models,coherence_values,num_topics_range,model_dir):
    '''
    Saves top n models below a certain number of topics.  
    '''
    n = 3
    cutoff = 15
    print('Now getting top %d models at max %d topics' %(n,cutoff))
    seq = get_top3_scores_and_index(coherence_values,num_topics_range,cutoff)
    top3_scores_idx = heapq.nlargest(3, seq)
    for i,(score,index) in enumerate(top3_scores_idx):
        optimal_topics = num_topics_range[index[1]]
        model = models[index[0]][index[1]]
        with open('%s/model_%stopics_score%s.pkl'%(model_dir,optimal_topics,str(score)[2:5]),'wb') as f:
            pickle.dump(model, f)
        print("Model %d: Number of topics = %d, coherence score = %.3f, in Trial %s" %(i,optimal_topics,score,index[0]+1))
    print('Top 3 models saved as .pkl files in %s' %model_dir)
...
def get_coherence_values(corpus, id2word, texts, num_topics_range, num_trials):
    ...
    """
    print("Running %s trials up to %s topics each to determine optimal number of topics." %(num_trials,num_topics_range[-1]))
    tic = time.time()
...
def main():
    ...
    get_topn_models(models,coherence_values,num_topics_range,model_dir)
